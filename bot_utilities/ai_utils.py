...
openai_client = AsyncOpenAI(
    api_key=os.environ.get("CHIMERA_GPT_KEY"), base_url="https://api.naga.ac/v1"
)


async def sdxl(prompt):
    response = await openai_client.images.generate(
        model="sdxl", prompt=prompt, n=1, size="1024x1024"  # images count
    )
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    params={
                        "query": search_query,
                        "maxNumResults": search_results_limit,
                    },
                ) as response:
