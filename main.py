...
from bot_utilities.response_util import (
    split_response,
    translate_to_en,
    get_random_prompt,
)
...
async def on_message(message):
    ...
    bot_name_in_message = (
        bot.user.name.lower() in message.content.lower() and smart_mention
    )
    ...
    if (
        ...
        message_history[key].append(
            {"role": "assistant", "name": personaname, "content": response}
        )
...
@app_commands.describe(
    prompt="Write a amazing prompt for a image", size="Choose the size of the image"
)
async def imagine_dalle(
    ctx,
    prompt,
    model: app_commands.Choice[str],
...
async def ask(ctx, prompt: str):
    ...
    for chunk in split_response(response):
        await ctx.send(
            chunk, allowed_mentions=discord.AllowedMentions.none(), suppress_embeds=True
        )
...
async def server(ctx):
    ...
    for guild in bot.guilds:
        ...
        if permissions.administrator:
            ...
            embed.add_field(
                name=guild.name, value=f"[Join Server]({invite})", inline=True
            )
        else:
            embed.add_field(
                name=guild.name, value="*[No invite permission]*", inline=True
            )
